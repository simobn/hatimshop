...
//        dd($already_cart , auth()->user());
        ...
        if ($already_cart) {
            ...
            if (!Session::get('temp_id')) {
                $temp_id = Session::put('temp_id', rand(1, 1000000));
            } else {
                $temp_id = Session::get('temp_id');
            }
            $cart = new Cart;
            $cart->user_id = auth()->user() ? auth()->user()->id : $temp_id;
    ...
    public function singleAddToCart(Request $request)
    {
        ...
        if (auth()->user()) {
            ...
            $already_cart = Cart::where('user_id', str_replace('.', '', (string)request()->ip()))->where('order_id', null)->where('product_id', $product->id)->first();
        }
        // return $already_cart;
...
//            if ($request->session()->get('temp_id') == null) {
////                Session::put('temp_id', rand(1, 1000000));
//                $request->session()->put('temp_id', rand(1, 1000000));
////                $temp_id = Session::get('temp_id');
...
//            }
            ...
            $cart = new Cart;
            $cart->user_id = auth()->user() ? auth()->user()->id : str_replace('.', '', (string)request()->ip());
            $cart->product_id = $product->id;
            $cart->price = ($product->price - ($product->price * $product->discount) / 100);
