<!-- Start Shop Newsletter  -->
<div class="p-6 container md:w-2/3 xl:w-auto mx-auto flex flex-col xl:items-stretch justify-between xl:flex-row">
    <div class="xl:w-1/2 md:mb-14 xl:mb-0 relative h-auto flex items-center justify-center">
        <img src="{{asset('images/newsletter.png')}}"
             alt="Envelope with a newsletter" role="img" class="h-full xl:w-full lg:w-1/2 w-full"/>
    </div>
    <div class="w-full xl:w-1/2 xl:pl-40 xl:py-28">
        <h1 class="text-2xl md:text-4xl xl:text-5xl font-bold leading-10 text-gray-800 mb-4 text-center xl:text-left md:mt-0 mt-4">
            {{__("subscribe")}}</h1>

        <p class="text-base leading-normal text-gray-600 text-center xl:text-left">{{__("subscribe to have the last news about us , coupons , promotions etc ..")}}</p>

        <div class="w-full">
            <form action="{{route('subscribe',app()->getLocale())}}" method="post" class="flex items-stretch mt-12">
                @csrf
                <input
                    class="bg-gray-100 rounded-lg rounded-r-none text-base leading-none text-gray-800 p-5 w-4/5 border border-transparent focus:outline-none focus:border-gray-500"
                    type="email" placeholder="{{__("Email")}}"/>
                <button
                    class="w-32 rounded-l-none hover:bg-indigo-600  rounded text-base font-medium leading-none text-white p-5 uppercase focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700"
                    type="submit" style="background-color: #Ff5641">{{__("subscribe")}}
                </button>
            </form>

        </div>
    </div>
</div>

<!-- End Shop Newsletter -->
